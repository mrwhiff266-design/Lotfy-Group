...
export default function NewOrderPage() {
  ...
  useEffect(() => {
    const searchCustomers = async () => {
      ...
      try {
        const res = await fetch(`/api/customers?query=${encodeURIComponent(customerName)}`);
        const data = await res.json();
  ...
  // FIX: Explicit types for 'value' to satisfy "No Any" rule
  const handleItemChange = (index: number, field: keyof OrderItem, value: string | number) => {
    const newItems = [...items];
    // FIX: Clean object update without casting errors
    newItems[index] = { ...newItems[index], [field]: value };
    setItems(newItems);
  };

  const addItem = () => setItems([...items, { sku: '', quantity: 1, price: 0, productId: '', name: '' }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch('/api/customers', {
        ...
        body: JSON.stringify({ name: customerName, phone, companyName }),
      });
